...
rm_subdir=set(['training_state'])
rm_file=set(['lr_scheduler_best.pkl', 'optimizer_best.pkl'])

confirm=None
for (dirname, subdirlist, filelist) in os.walk(basedir):

    # check if this is a sockeye-recipes directory
    if 'cmdline.log' in filelist and 'args.yaml' in filelist:

        if confirm != 'a':
            print("Do you want to clean: %s ?" %dirname),
            confirm = input("  [y]es, [n]o, yes to [a]ll ")
        ...
        if confirm == 'y' or confirm == 'a':
            ...
            # remove decode.output
            for f in filelist:
                if f.startswith('decode.output'):
                    os.remove(dirname+"/"+f)
                    print("  rm "+f)
